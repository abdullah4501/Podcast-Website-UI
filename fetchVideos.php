...
function getChannelStats($channelId, $apiKey)
{
    ...
    if (!isset($data['items'][0]['statistics'])) return [];
...
function getPlaylistVideoCount($playlistId, $apiKey)
{
    if (!$playlistId || !$apiKey) return 0;
    ...
    return $response['items'][0]['contentDetails']['itemCount'] ?? 0;
}
